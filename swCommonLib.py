...
def convertSelectWhereListToSelectWhere(
        fieldList,
        tableName,
        selectList,
        whereList):
...
def helper(data, classFile, outFile, otherFile):
    ...
    # get the name of the class from the FROM field so for example
    # Orion.VIM.VMStatstics = vimVMStatistics
    ...
    helperPrint(
        "\t\t\t%s = self.sendRequest(Type='GET', URL= '/SolarWinds/InformationService/v3/Json/Query?query=' + selectQuery, status=200)\n" %
        currStr,
        outFile)
    ...
    helperPrint(
        "\t\t\tthisObj = %s(connection=self,data=obj)\n" %
        swClassName,
        outFile)
    helperPrint(
        "\t\t\tself.%sObjs.append(thisObj)\n" %
        swFunctionName,
        outFile)
